...
contract FlightInsuranceTest is Test {
    ...
    function setUp() public {
        vm.startPrank(owner);
        idrx = new IDRX();
        bool success = idrx.transfer(user, 10_000_000 * 10 ** 18);
        require(success, "Initial token transfer to user failed");
        vm.stopPrank();

        flightInsurance = new FlightInsurance(address(idrx), owner);
    }
    
    function test_MintTicketSuccessfully() public {
        vm.startPrank(user);
        idrx.approve(address(flightInsurance), TICKET_PRICE);
    ...
    function test_Refunds5PercentFor1HourDelay() public { 
        ...
        assertEq(idrx.balanceOf(user), (10_000_000 * 10 ** 18) - TICKET_PRICE + expectedRefund);
    ...
    function test_Refunds95PercentForCancellation() public { 
        test_MintTicketSuccessfully();
        uint256 userBalanceBeforeRefund = idrx.balanceOf(user);
        vm.startPrank(owner);
        flightInsurance.reportFlightStatus(1, FlightInsurance.Status.Cancelled, 0);
        vm.stopPrank();
        uint256 expectedRefund = (TICKET_PRICE * 95) / 100;
        assertEq(idrx.balanceOf(user), userBalanceBeforeRefund + expectedRefund);
        FlightInsurance.TicketInfo memory info = flightInsurance.getTicketInfo(1);
        assertEq(uint(info.status), uint(FlightInsurance.Status.Refunded));
    }
    ...
    function test_OwnerCanWithdrawFunds() public { 
        ...
        vm.stopPrank();
        assertEq(idrx.balanceOf(address(flightInsurance)), 0);
        assertEq(idrx.balanceOf(owner), ownerBalanceBefore + contractBalance);
    }
